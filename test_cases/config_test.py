    ...
    if report.when == "call" and report.failed:
        ...
        if driver:
            try:
                ...
                print(f"[INFO] Screenshot attached for test: {item.name}")
            except Exception as e:
                print(f"[ERROR] Failed to attach screenshot: {e}")
        else:
            print(f"[WARNING] No driver found to take screenshot for test: {item.name}")
